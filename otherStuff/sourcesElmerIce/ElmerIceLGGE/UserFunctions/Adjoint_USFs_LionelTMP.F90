...
FUNCTION AsquareB(Model,nodenumber,VarIn) RESULT(VarOut)
  ...
  TYPE(Model_t) :: Model
  INTEGER :: nodenumber
  REAL(kind=dp) :: VarIn(2),VarOut, Ga
  CHARACTER(LEN=MAX_NAME_LEN) :: USFName='AsquareBo'
  LOGICAL :: Found

  Ga=GetCReal( Model % Constants, 'Friction weight',Found )
  IF (.NOT.Found) &
    CALL FATAL(USFName,'<Friction weight> not found')

  VarOut = VarIn(1)*VarIn(1)*VarIn(2)/Ga
...
FUNCTION Derivative_AsquareB(Model,nodenumber,VarIn) RESULT(VarOut)
  ...
  TYPE(Model_t) :: Model
  INTEGER :: nodenumber
  REAL(kind=dp) :: VarIn(3),VarOut, Ga
  CHARACTER(LEN=MAX_NAME_LEN) :: USFName='AsquareBo'
  LOGICAL :: Found
  ...
  VarOut = 2.0*VarIn(1)*VarIn(2)*VarIn(3)/Ga
