!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! USF_Damage.f90
! Last modif : 12 October 2015
! Computes the evolution of damage and computes the Enhancement factor of the Glen's law
! as a function of damage evolution 
! This solver should work both in FS and SSA. Flow solution name has to be given in Equation section
! (Flow Solution for FS and SSABulkVelocity for SSA)
!
!
...
FUNCTION SourceDamage (Model, nodenumber, D) RESULT(Source)
   ...
   TYPE(ValueList_t), POINTER :: Material, Constants, Eq
   ...
   REAL (KIND=dp) :: SigmaI, SigmaII, Chi_prov, Chi, B, sigmath, pwater
   ...
   CHARACTER(LEN=MAX_NAME_LEN) :: FlowSolFlag
   ...
   ELSE
      CALL INFO('Damage Source', 'PSeaD not associated, basal pressure not taken into account in damage formation' )
      CALL INFO('Damage Source', 'Taking default value PSeaD=0.0')
   END IF
   ...
   FlowSolFlag = ListGetString (Eq, 'Flow Solution Name', GotIt)
 
   IF (.NOT. GotIt) THEN
   CALL FATAL ('Damage Source','Flow Solution Name has to be precised in section &
        Equation. SSABulkVelocity if SSA, Flow Solution if NS')
   END IF

   FlowVariable => VariableGet( Model % Variables, FlowSolFlag )
   IF ( ASSOCIATED( FlowVariable ) ) THEN
      FlowPerm    => FlowVariable % Perm
      FlowValues  => FlowVariable % Values
   ELSE
      CALL FATAL('Damage Source', 'Flow Solution not associated')
   END IF
  ...
  ! Need Cauchy Stress and Deviatoric Stress 
   ...
   ! Damage Criterion
   Chi_prov = 1.0_dp / (1.0_dp - D) * (SigmaI + pwater) - stress_threshold
   Chi = MAX(Chi_prov,0.0_dp)
