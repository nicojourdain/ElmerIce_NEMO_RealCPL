...
!------------------------------------------------------------------------------
  ...
  END IF
    ...
    IF(.NOT. Found) THEN
       CALL INFO("FrontDisplacement","Can't find CalvingOccurs Logical, assuming false")
       CalvingOccurs = .FALSE.
    END IF

  IF ( Found .AND. .NOT. CalvingOccurs ) THEN 
       CALL INFO("FrontDisplacement","No Calving, leaving FrontDisplacement Solver", level=3)
  RETURN
  ENDIF
...
!TEST
  IF(FirstTime) THEN
     ...
     UpdateSol => VariableGet( Solver % Mesh % Variables, 'Mesh Update' )
...
!  END IF
 ...
 ! GAG : I don't think this has to be done here...
