! Calcul du gradient de la fonction cout du probleme inverse Adjoint
!   par rapport a  un enhencement de la viscosité (i.e mu=E mu_0) (cf These Mathieu Morlighem)
...
!
!  !!!!!!  Regularisation ajoutee comme Lambda * int_bed (DE/Dx)^2  !!!!!!!
!
...
!     - Flow Solution Name = String ; le nom de la variable du probleme de Neumann ("Flow Solution" par defaut)
!     - Adjoint Solution Name = String ; le nom de la variable du probleme de Dirichlet ("VeloD" par defaut)
...
!     - SquareFormulation = Logical ; True si la E defini comme alpha^2 et optimisation sur alpha pour assurer une viscosite positive
...
!
! *****************************************************************************
SUBROUTINE DJDE_Adjoint_Z( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
!******************************************************************************
  USE DefUtils
  IMPLICIT NONE
...
!!!! Autres variables
  ...
  real(kind=dp),allocatable :: NodeDJ(:),NodalVeloN(:,:),NodalVeloD(:,:),NodalMu0(:),m(:),cs(:)
  ...
  integer :: i,j,k,l,t,n,NMAX,NActiveNodes,DIM
  integer :: up
  ...
  If (Firsttime) then
      WRITE(SolverName, '(A)') 'DJDE_Adjoint_Z'
 ...
 ! Get variables needed by the Solver
    ...
    DO t=1,Solver % NumberOfActiveElements
          ...
          Do i=1,n
             ...
             !!! le gradient = -tr(e^Lambda.e^u)  cf these Morlighem
             ...
             if (m(i).ne.1.0_dp) then
                     ...
                     NodeDJ(i)=NodeDJ(i)*(SecInv)**(0.5_dp*(m(i)-1))
             end if
