...
!   par rapport a la viscosite (cf These Mathieu Morlighem)
...
!
! *****************************************************************************
SUBROUTINE DJDMu_Adjoint_Z( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
!******************************************************************************
  USE DefUtils
  IMPLICIT NONE
...
!!!!! variables Elmer
  ...
  INTEGER, POINTER :: Perm(:), VeloNPerm(:),VeloDPerm(:),GradPerm(:)
...
!!!! Autres variables
  ...
  real(kind=dp),allocatable :: NodeDJ(:),NodalVeloN(:,:),NodalVeloD(:,:),m(:),cs(:)
  ...
  save VisitedNode,db,NodeDJ,Basis,dBasisdx,NodalVeloN,NodalVeloD,m,cs
  SAVE TopPointer,BotPointer,Upointer,DownPointer

  If (Firsttime) then
      WRITE(SolverName, '(A)') 'DJDMu_Adjoint_Z'
 ...
 ! Get variables needed by the Solver
    ...
    DO t=1,Solver % NumberOfActiveElements
          ...
          Do i=1,n
             ...
             !!!! si m != 1; DJ/Dmu=DJ/Dmu_eff * Dmu_eff/Dmu; m_eff=mu*gamma^(m-1)
             ...
             !!! si viscosity=mu*mu; DJ/Dmu=DJ/Dviscosity*Dviscosity/Du
...
!------------------------------------------------------------------------------
END SUBROUTINE DJDMu_Adjoint_Z
!------------------------------------------------------------------------------
