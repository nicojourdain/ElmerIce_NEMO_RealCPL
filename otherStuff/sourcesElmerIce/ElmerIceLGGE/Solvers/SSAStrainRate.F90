...
! *
! *****************************************************************************
!> Module containing a solver for computing the strain rate for the SSA solution
!> The SSA Velocity dimension is DIM-1 or DIM 
!> 1D SDOFs = 1 (E11)                                   
!> 2D SDOFs = 4 (E11, E22, E33, E12)                         
!------------------------------------------------------------------------------
   RECURSIVE SUBROUTINE SSAStrainRate( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
     ...
     CHARACTER(LEN=MAX_NAME_LEN) :: VarName, StrainRateVariableName
     CHARACTER(LEN=MAX_NAME_LEN) :: SolverName = 'SSAStrainRate'
...
!------------------------------------------------------------------------------
!  Read the SSAVelocity solution
!------------------------------------------------------------------------------
      
     FlowVariable => VariableGet( Solver % Mesh % Variables, 'SSAVelocity' )
     IF ( ASSOCIATED( FlowVariable ) ) THEN
       FlowPerm    => FlowVariable % Perm
       FlowValues  => FlowVariable % Values
       vDOFs     =  FlowVariable % DOFs
     ELSE
...
!------------------------------------------------------------------------------
      ...
      IF ( STDOFs /=1 ) THEN
              CALL FATAL( SolverName, 'DOF must be equal to 1' )
      END IF

      StrainRateVariableName = GetString( Model % Constants, 'StrainRate Variable Name', GotIt )    
      ...
      ! dim = 1 or 2 EiiDOFs = 1
      ! dim = 2 or 3 EiiDOFs = 4 
      IF ((vDOFs==1).AND.(EiiDOFs/=1)) CALL FATAL( SolverName, 'Strain Rate dimension should be 1' )
      IF ((vDOFs==2).AND.(EiiDOFs/=4)) CALL FATAL( SolverName, 'Strain Rate dimension should be 4' )
...
!------------------------------------------------------------------------------
          ...
          LocalVelo = 0.0d0
          DO i=1, vDOFs
             LocalVelo(i,1:n) = FlowValues(vDOFs*(FlowPerm(NodeIndexes(1:n))-1) + i)
          END DO
...
!
     ...
     REAL(KIND=dp) :: LGrad(3,3), SR(3,3),  Eij(4)
