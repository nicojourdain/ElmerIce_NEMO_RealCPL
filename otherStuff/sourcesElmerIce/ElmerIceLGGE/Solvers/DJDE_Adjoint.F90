!! Calcul du gradient nodal de la fonction cout du probleme inverse Adjoint 
!   par rapport a a un enhencement de la viscosité (i.e mu=E mu_0)
!              (cf These Mathieu Morlighem)
!
...
!     - Flow Solution Name = String ; le nom de la variable du probleme direct ("Flow Solution" par defaut)
!     - Adjoint Solution Name = String ; le nom de la variable du probleme Adjoint ("Adjoint" par defaut)
...
!     - SquareFormulation = Logical ; True si E defini comme alpha^2
...
!       -CostSolver_Adjoint.f90: pour le calcul de la fonction cout;
!          !!ATTENTION!! : Pas de regularisation pour l'instant mettre Lamda=Real 0.0 pour le calcul de la fonction cout
!       -Optimize_m1qn3[Serial/Parallel].f90: pour l'optimisation !! Solver a executer aussi dans le body (contrairement a l'optimisation de Beta)
!
! *****************************************************************************
SUBROUTINE DJDE_Adjoint( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
!******************************************************************************
  USE DefUtils
  IMPLICIT NONE
...
!!!!! variables Elmer
  TYPE(Variable_t), POINTER :: GradVariable, Variable, VeloSolN,VeloSolD,Mu0Sol
  REAL(KIND=dp), POINTER :: GradValues(:),VelocityN(:),VelocityD(:),Values(:),Mu0(:)
  INTEGER, POINTER :: GradPerm(:), VeloNPerm(:),VeloDPerm(:),Perm(:),Mu0Perm(:)
  CHARACTER(LEN=MAX_NAME_LEN) ::GradSolName,NeumannSolName,DirichletSolName,VarSolName,Mu0SolName
...
!! autres variables
  ...
  If (Firsttime) then
      ...
      WRITE(SolverName, '(A)') 'DJDE_Adjoint'
